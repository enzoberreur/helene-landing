      ...
      {sent ? (
        ...
        <>
          ...
          <div className="flex gap-2">
            <button onClick={() => setOpen(false)} className="flex-1 py-2.5 rounded-xl text-xs font-medium" style={{ background: theme.background, color: theme.textSecondary }}>{t('common.cancel')}</button>
            <button onClick={handleSend} disabled={!feedback.trim()} className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-white disabled:opacity-30" style={{ background: theme.dark }}>{t('common.send')}</button>
          </div>
        </>
      )}
    </div>
    ...
    <div className="flex flex-col flex-1 min-h-0 px-6" style={{ background: theme.background }}>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        <div className="rounded-3xl p-4 mb-4" style={{ background: theme.surface }}>
          <p className="text-xs font-semibold mb-3 uppercase tracking-wide" style={{ color: theme.textLight }}>{t('profile.my_journey')}</p>
        ...
        {profile.selectedSymptoms.length > 0 && (
          <div className="rounded-3xl p-4 mb-4" style={{ background: theme.surface }}>
            <p className="text-xs font-semibold mb-3 uppercase tracking-wide" style={{ color: theme.textLight }}>{t('profile.my_symptoms')}</p>
        ...
        {/* Community Identity */}
        <div className="rounded-3xl p-4 mb-4" style={{ background: theme.surface }}>
          <p className="text-xs font-semibold mb-3 uppercase tracking-wide" style={{ color: theme.textLight }}>{t('profile.community_identity')}</p>
